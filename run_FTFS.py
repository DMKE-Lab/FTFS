...
def run_DATASET(args):
    ...
    layers = args.layers
    gw_beta = args.gw_beta
    gw_epoch = args.gw_epoch
    unsup = args.unsup
    ...
    if which_file == 0:
        ...
        embedding_name = 'embedding_F_DICEWS'
    else:
        filename = 'data/YAGO-WIKI50K/'
        dataset_name = 'YAGO-WIKI50K'
        embedding_name = 'embedding_F_YAGO-WIKI50K'



    train_pair, dev_pair, adj_matrix, r_index, r_val, t_index, adj_features, rel_features, time_features = load_data(filename, unsup=unsup)
    ...
    feature = torch.load('data/{}/features.pt'.format(dataset_name)).to('cpu')
    time_features = torch.load('{}/time_featuers.pt'.format(embedding_name)).to('cpu')
    rel_features = torch.load('{}/out_feature_rel.pt'.format(embedding_name)).to('cpu')
    ent_features = torch.load('{}/out_feature_ent.pt'.format(embedding_name)).to('cpu')
